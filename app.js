...
function mostrarAviso(mensagem, tipo = 'erro') {
    ...
    toast.style.cssText = `background: ${tipo === 'sucesso' ? '#00e5b0' : '#ff4d4d'}; color: ${tipo === 'sucesso' ? '#002b22' : '#ffffff'}; padding: 12px 20px; border-radius: 8px; font-weight: bold; display: flex; align-items: center; gap: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.3); opacity: 0; transform: translateY(20px); transition: all 0.3s; z-index: 99999; cursor: pointer;`;
...
document.addEventListener('click', function(e) { if (!e.target.closest('.custom-select-wrapper')) { const opts = document.getElementById('custom-options-sup'); if (opts) opts.classList.remove('show'); } });
function selecionarFiltroSupervisorCustom(id, nome) { document.getElementById('filtro-sup-texto').innerText = nome; filtroSupIdSelecionado = id; document.getElementById('custom-options-sup').classList.remove('show'); carregarPedidos(true); }
...
async function fazerLogin() {
    ...
    const erroCard = document.getElementById('login-erro-card');
    ...
    try {
        ...
        mostrarAviso('Credenciais inválidas.', 'erro'); 
...
function mostrarTela(idTela) {
    ...
    ['tela-pedidos', 'tela-fazer-pedido', 'tela-gerenciar-catalogo', 'tela-gerenciar-usuarios'].forEach(tela => { const el = document.getElementById(tela); if (el) el.style.display = (tela === idTela) ? 'block' : 'none'; });
...
function renderizarVitrine() {
    ...
    vitrine.innerHTML = ''; if (filtrados.length === 0) { vitrine.innerHTML = '<div style="color: var(--cor-secundaria); padding: 20px;">Nenhum material encontrado.</div>'; return; }
...
async function carregarLojasSelect() {
    const sel = document.getElementById('select-loja'); if(!sel) return; let q = supabaseClient.from('lojas').select('id, nome').order('nome'); if (usuarioLogado.cargo === 'Supervisor') q = q.eq('supervisor_id', usuarioLogado.id);
...
async function carregarSupervisoresModal() {
    const selects = ['loja-supervisor', 'edit-loja-supervisor']; const { data } = await supabaseClient.from('usuarios').select('id, nome').eq('cargo', 'Supervisor').order('nome');
...
async function carregarFiltroSupervisoresCustom() {
    const caixaOpts = document.getElementById('custom-options-sup'); if(!caixaOpts) return; const { data } = await supabaseClient.from('usuarios').select('id, nome').eq('cargo', 'Supervisor').order('nome');
    caixaOpts.innerHTML = `<div class="custom-option" onclick="selecionarFiltroSupervisorCustom('', 'Todos os Supervisores')">Todos os Supervisores</div>`; data.forEach(s => { caixaOpts.innerHTML += `<div class="custom-option" onclick="selecionarFiltroSupervisorCustom('${s.id}', '${s.nome}')">${s.nome}</div>`; });
}
...
function renderizarVitrinePedido() {
    ...
    filtrados.forEach(grupo => {
        ...
        grupo.variacoes.forEach(v => {
            ...
            variacoesHtml += `<div class="variacao-item"><div style="display:flex; flex-direction:column; max-width: 65%;"><span class="var-nome">${v.nome_var === 'Único' ? 'Unidade Padrão' : v.nome_var}</span><span class="var-estoque" style="${classeFalta}">${textoEstoque}</span></div><div class="qtd-selector" style="padding: 2px;"><button type="button" class="qtd-btn" style="width:25px;height:25px;font-size:16px;" onclick="alterarQtdPedido('${v.id}', -1)">-</button><input type="number" class="qtd-input" id="qtd-pedido-${v.id}" value="${v.qtdSelecionada}" readonly style="width:30px;font-size:14px;"><button type="button" class="qtd-btn" style="width:25px;height:25px;font-size:16px;" onclick="alterarQtdPedido('${v.id}', 1)">+</button></div></div>`;
        }); variacoesHtml += '</div>';
...
async function salvarSolicitacao() {
    ...
    try {
        ...
        mostrarAviso('Pedido Enviado com Sucesso!', 'sucesso'); document.getElementById('select-loja').value = ''; arquivosBancada = []; renderizarMiniaturasBancada(); memoriaCatalogoPedido.forEach(item => item.qtdSelecionada = 0); mostrarTela('tela-pedidos');
...
function abrirModalVerPedido(id) {
    ...
    listaItensRaw.split(',').forEach(item => { listaHtml += `<li style="background: rgba(255,255,255,0.05); padding: 8px 12px; margin-bottom: 5px; border-radius: 6px; border-left: 3px solid var(--primary);"><strong style="color:#fff;">${item.trim()}</strong></li>`; }); 
    listaHtml += `</ul>`;

    let reversaHtml = '';
    if (reversaJson) {
        reversaHtml += `<div style="background: rgba(168, 85, 247, 0.1); border: 1px solid rgba(168, 85, 247, 0.3); padding: 12px; border-radius: 8px; margin-bottom: 15px;">
                            <p style="margin: 0 0 10px 0; color: #a855f7; font-size: 12px; font-weight: bold;"><i data-lucide="rotate-ccw" class="lucide-sm"></i> LOGÍSTICA REVERSA SOLICITADA</p>
                            <ul style="list-style: none; padding: 0; margin: 0;">`;
        reversaJson.itens.forEach(rev => {
            reversaHtml += `<li style="font-size: 13px; color: #fff; margin-bottom: 5px;"><strong>${rev.qtd}x ${rev.nome}</strong> <br><span style="color:var(--cor-secundaria); font-size: 11px;">Motivo: ${rev.motivo}</span></li>`;
        });
        reversaHtml += `</ul></div>`;
    }
        ...
        for (let input of inputs) {
            ...
            if (qtdEnviada < qtdOriginal) {
                ...
                if (catData) {
                    ...
                    promessasBD.push(supabaseClient.from('logs_estoque').insert([{ material: nomeItem, quantidade_movimentada: diferenca, responsavel: usuarioLogado.nome, motivo: 'Estorno: Ruptura no Despacho' }])); 
                }
